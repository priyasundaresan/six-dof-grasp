...
class PoseDataset(Dataset):
	def __init__(self, dataset_dir, transform, img_height=136, img_width=136, gauss_sigma=5):
            ...
            for i in range(len(os.listdir(img_folder))-1):
                ...
                kpts = np.load(os.path.join(labels_folder, '%05d.npy'%i))
                if len(kpts) < 3:
                    continue
                axis_kpts = kpts[:-1]
...
if __name__ == '__main__':
	dset = PoseDataset('/host/datasets/acquis_rot_recenter/test', transform)
	for i in range(50):
		img, img_np, gauss, rot = dset[i]
		print(i, np.degrees(rot.cpu().numpy()))
